...
import { LlmModelSchema, type LlmModel, type ModelCacheStatus } from "../../core/platform"
...
interface CacheReadResult extends ModelCacheStatus {
  readonly data: ReadonlyArray<LlmModel> | null
}
  ...
  "model-analysis/LlmCatalogCache",
  {
    effect: Effect.gen(function* () {
      ...
      const read = Effect.fn("LlmCatalogCache.read")(function* (maxAgeSeconds?: number) {
        ...
        if (!exists) {
          return emptyReadResult(location, ttlSeconds, snapshotCount, false)
        }
        ...
        if (Either.isLeft(decoded)) {
          return {
            ...emptyReadResult(location, ttlSeconds, snapshotCount, true),
            valid: false,
            stale: true,
...
const toStatus = (cache: CacheReadResult): ModelCacheStatus => ({
